...
+ Epsilon-greedy GBFS --- *alt* ([h,*], [Random])
  + Sometimes runs a random search
+ Type-based GBFS ---- *alt* ([h,*], <g,h>)
  + Sometimes runs a depth-balanced search
+ GBFS-LE (local expansion)
  + Runs a randomized search when local minima is detected
  + GBFS-LS (local search)
  + GBFS-MRW (Monte-Carlo Random Walk)
...
+ e.g., water replacing the air in a porous rock
#+end_span6
#+end_row-fluid
#+end_container-fluid

** Bond Invasion Percolation
...
+ Every edge has a unique randomly assigned value
+ Mark the initial node
+ Mark the node connected to the marked node with the least edge value
+ This description is equivalent to the Prim's method for Minimum Spanning Tree

# figure

** How it is implemented

Instead of assigining the random values to all edges (which is infeasible in the implicit graph in classical planning)

Assign a random value to an edge on demand
...
* Discussion : Advantage over BWFS
...
* Discussion : Additional Evaluation: BWFS and GBFS-W in (Lipovetzky AAAI17)
