...
+ Because *depth (d,D)* and *width (b,B)* has the different effect
+ Effects mostly combine
#+end_span5
#+end_row-fluid
#+end_container-fluid
...
+ _/BFWS is inter-plateau [w,h]/_
+ Effects are (mostly) orthogonal
+ *Effects are expected to combine with both enabled*
+ Difference between BIP and IW
  + *IW assumes propositional states*, BIP does not
#+end_span6
#+end_row-fluid
#+end_container-fluid
